...
public class TodoListView {
	...
	public void startView() {
		...
		do {
			...
			try {
				...
				input = -1; // 첫 반복에서의 종료 방지
				
			} catch(IOException e) {
				System.out.println("### 입/출력 관련 예외 발생 ###");
				e.printStackTrace();
				
			} catch(Exception e) { // 기타 예외 추적
				e.printStackTrace();
			}
	...
	private void todoListFullView() {
		...
		// (List)		(int) <- 반환값은 1개만 가능 ~> Map으로 묶어 한덩어리로 가져오기
	...
	private void todoAdd() throws IOException {
		...
		System.out.println("세부 내용 작성 (입력 종료시 !wq 작성후 엔터)");
		System.out.println("--------------------------------------------");
	...
	/**
	 * 할 일 수정
	 * - index 번호를 입력 받아 정상 범위내의 index 값인지 확인
	...
	private void todoUpdate() throws NumberFormatException, IOException {
		...
		System.out.println("수정할 세부 내용 작성 (입력 종료시 !wq 작성후 엔터)");
		System.out.println("--------------------------------------------");
	...
	private void todoDelete() throws NumberFormatException, IOException {
		...
		String title = service.todoDelete(index); //제목/null 반환
